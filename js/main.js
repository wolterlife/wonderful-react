...
const d = document;
const itemBox = d.querySelectorAll('.item'); // div for items
const price = d.getElementById('price'); // total price block
cartView = d.getElementsByClassName('cart-view')[0];
if (localStorage.getItem('cart')) {
  // set total when page loaded. if null - ignore
  totalJson = JSON.parse(localStorage.getItem('cart')).total;
  price.innerHTML = `${totalJson} руб.`;
}

function cartDec(e) {
  const namePizza = e.parentNode.parentNode.querySelector('td');
  const cartData = getCartData(); // get data and create obj if cart is empty
...
function addToCart(e) {
  ...
  const cartData = getCartData() || { total: 0 }; // get data and create obj if cart is empty
  const parentBox = e.parentNode.parentNode; // parent button "+"
  const itemId = e.getAttribute('data-id'); // info about pizza
  const itemTitle = parentBox.querySelector('.item-name').innerHTML;
  const itemPrice = parentBox.querySelector('.item-price').innerHTML;
  ...
  if (cartData.hasOwnProperty(itemId)) {
    // if item in cart add +1 to value
    cartData[itemId][2] += 1;
  } else {
    // if this item don't in cart
...
function openCart(showBaskset) {
  ...
  if (cartData) {
    finalCartList =
      '<table class="shopping_list"><tr><th>Заказ</th><th>Цена</th><th>Кол-во</th><th></th></tr>';
    for (const items in cartData) {
      // form table with result
      if (items !== 'total') {
        // When we have end of the table items = total
        finalCartList += '<tr>';
        ...
        finalCartList +=
          '<td>' +
          '<img src="img/minus-box-outline.svg" alt="" onclick="cartDec(this)"/>' +
          '</td>';
        finalCartList += '</tr>';
      }
    }
    finalCartList += '</table>';
    d.getElementsByClassName('btn-in-pop')[0].style.display = 'flex';
    cartView.innerHTML = finalCartList;
    if (showBaskset) {
    }
  } else {
    d.getElementsByClassName('btn-in-pop')[0].style.display = 'none';
